...
It can be used three ways:
  1. CLI:        ``python cdp_helper.py open https://example.com``
  2. Imported:   ``from cdp_helper import ChromeSession; ...``
  3. By the agent via execute_code / terminal (no Hermes internals needed).
...
NOTE: page-level CDP methods (``Runtime.evaluate``, ``Page.captureScreenshot``,
``Input.dispatchMouseEvent`` ...) require a *session*, which means attaching to
a specific target (tab). The helpers here auto-create/attach a tab so you
don't manage sessionIds by hand.
"""
...
def _cmd(args) -> int:
    port = args.port or int(__import__("os").environ.get("CDP_PORT", DEFAULT_PORT))
    if args.action == "open":
